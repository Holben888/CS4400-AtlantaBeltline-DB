...
var App = (function () {
	...
	// (3:4) {#each headers as header, index}
	function create_each_block_2(ctx) {
		...
		return {
			...
			p(changed, new_ctx) {
				ctx = new_ctx;
				if ((changed.headers) && text0_value !== (text0_value = ctx.header)) {
	...
	function create_each_block(ctx) {
		...
		return {
			c() {
				...
				text = createText("\n  ");
				tr.className = tr_class_value = "" + (ctx.index === ctx.selected && !ctx.noSelect ? 'selected' : '') + " svelte-165g8p4";
			...
			p(changed, new_ctx) {
				...
				if ((changed.selected || changed.noSelect) && tr_class_value !== (tr_class_value = "" + (ctx.index === ctx.selected && !ctx.noSelect ? 'selected' : '') + " svelte-165g8p4")) {
	...
	function create_fragment$3(ctx) {
		...
		return {
			...
			p(changed, ctx) {
				...
				if (changed.selected || changed.noSelect || changed.data) {
					each_value = ctx.data;
	...
	function instance$3($$self, $$props, $$invalidate) {
	  ...
	  let { sortEndpoint = '', sortParams = {}, setData = () => {} } = $$props;
	  let { selectItem = () => {} } = $$props;
	  let { selected = -1, data = [], headers = [], noSelect = false } = $$props;
	  ...
	  const sortCallback = async (header, sortAscending) => {
		...
		function click_handler({ index }) {
			return sortCallback(Object.keys(data[0])[index], true);
		}

		function click_handler_1({ index }) {
			return sortCallback(Object.keys(data[0])[index], false);
		}
		...
		$$self.$set = $$props => {
			...
			if ('sortParams' in $$props) $$invalidate('sortParams', sortParams = $$props.sortParams);
			...
			if ('noSelect' in $$props) $$invalidate('noSelect', noSelect = $$props.noSelect);
		};
	...
	class Table extends SvelteComponent {
		...
		get noSelect() {
			return this.$$.ctx.noSelect;
		}

		set noSelect(noSelect) {
			this.$set({ noSelect });
			flush();
		}
	}
	...
	function create_fragment$4(ctx) {
		var h1, text1, table0, tr, td0, text3, td1, text4, td2, text6, td3, select0, option0, option1, option2, option3, option4, text12, td4, text14, td5, select1, option5, option6, option7, option8, text19, button0, text21, button1, text23, button2, text25, br0, text26, br1, text27, current, dispose;
		...
		return {
			...
			p(changed, ctx) {
				...
				if (changed.tableHeaders) table1_changes.headers = ctx.tableHeaders;
	...
	/* src/transit/take.html generated by Svelte v3.0.0-beta.3 */

	function create_fragment$6(ctx) {
		var h1, text1, text2, form, current, dispose;
		...
		var table = new Table({
			...
			setData: ctx.setData,
			selected: ctx.transitSelected,
			selectItem: ctx.selectTransit,
			headers: ctx.tableHeaders
		}
		});
		...
		return {
			c() {
				h1 = createElement("h1");
				h1.textContent = "Take a Transit";
				text1 = createText("\n\n");
				table.$$.fragment.c();
				text2 = createText("\n\n");
				form = createElement("form");
				form.innerHTML = `<input type="date" name="date">
			  <button class="general-button" onclick="log()" style="margin-left: 20px;">
			    Log Transit
			  </button>`;
				form.id = "logTransit";
				dispose = addListener(form, "submit", preventDefault(ctx.logTransit));
			},
			...
			p(changed, ctx) {
				var table_changes = {};
				if (changed.tableData) table_changes.data = ctx.tableData;
				if (changed.setData) table_changes.setData = ctx.setData;
				if (changed.transitSelected) table_changes.selected = ctx.transitSelected;
				if (changed.selectTransit) table_changes.selectItem = ctx.selectTransit;
				if (changed.tableHeaders) table_changes.headers = ctx.tableHeaders;
	...
	function instance$6($$self, $$props, $$invalidate) {
	  ...
	  onMount(async () => {
	    const response = await fetch$1('/transit');
	    if (response && response.transits) { tableData = response.transits; $$invalidate('tableData', tableData); }
	  });

	  const setData = newData => {
	    if (newData.transits) { tableData = newData.transits; $$invalidate('tableData', tableData); }
	  };
	...
	/* src/transit/history.html generated by Svelte v3.0.0-beta.3 */
	...
	function create_fragment$7(ctx) {
		...
		return {
			c() {
				h1 = createElement("h1");
				h1.textContent = "Transit History";
				text_1 = createText("\n\n");
				table.$$.fragment.c();
			},
			...
			p(changed, ctx) {
				var table_changes = {};
				if (changed.tableData) table_changes.data = ctx.tableData;
				if (changed.sortParams) table_changes.sortParams = ctx.sortParams;
				if (changed.setData) table_changes.setData = ctx.setData;
				if (changed.tableHeaders) table_changes.headers = ctx.tableHeaders;
				table.$set(table_changes);
			},
	...
	function instance$7($$self, $$props, $$invalidate) {
	  ...
	  onMount(async () => {
	    sortParams = { username: $userStore.Username }; $$invalidate('sortParams', sortParams);
	    const response = await fetch$1('/transit/history', sortParams);
	    if (response && response.transits) { tableData = response.transits; $$invalidate('tableData', tableData); }
	  });
	...
	function create_fragment$8(ctx) {
		...
		var if_block = (ctx.location.pathname !== '/') && create_if_block$2(ctx);
			...
			p(changed, ctx) {
				if (switch_value !== (switch_value = ctx.currPageComponent)) {
					...
					if (switch_value) {
						...
						mount_component(switch_instance, div, text);
					} else {
	...
	function instance$8($$self, $$props, $$invalidate) {
	  ...
	  onDestroy(() => unsubscribePage());
		...
		return { currPageComponent, location };
	}
